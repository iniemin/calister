...
"""
    ...
    def START(message):
        msg = f"""
<blockquote><b>👋 Welcome di Calister Assistant <a href=tg://user?id={message.from_user.id}>{message.from_user.first_name} {message.from_user.last_name or ''}</a></b>

<b><u>[{nama_bot}](https://t.me/{bot.me.username})</u> adalah sebuah Bot multi client dengan multi bahasa. Yang dibuat untuk publik secara GRATIS tanpa persyaratan apapun.</b></blockquote>
<b>Created by {Ads()}</b>
"""
        return msg
                    ...
                    results=[
                        (
                            InlineQueryResultArticle(
                                title=f"Anda Belum Mendapatkan Akses {bot_username}",
        ...
        async def wrapper(client, cq, *args):
            ...
            if cq.from_user.id not in users:
                return await cq.answer(
                    f"Silakan Minta Akses Bot {bot_username} Agar Bisa Menggunakan Bot Ini",
                    True,
                )
            else:
